...
/*   Updated: 2024/03/27 11:56:55 by skhastag         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static t_list	*stack[4096];
	t_list			*temp;
	char			*line;

	line = NULL;
	temp = NULL;
	if (fd < 0 || fd >= 4096 || read(fd, &line, 0))
	{
		while (*(stack + fd))
		{
			temp = (*(stack + fd))->next;
			free((*(stack + fd))->content);
			free(*(stack + fd));
			*(stack + fd) = temp;
		}
		free(*(stack + fd));
	...
	create_stack(fd, &(*(stack + fd)));
	if (!*(stack + fd))
		return (NULL);
	line = get_line_str(*(stack + fd));
	clear_stack(&(*(stack + fd)));
	return (line);
}
